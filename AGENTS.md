...
分割取得に戻さないこと**——並び順が保証されず、読んでいる間の追加・削除で行が抜ける・重複する
（SQLエクスポートを流し込むと主キー違反になる）。主キーがあれば主キー順のキーセット方式
（`WHERE k > 前回の最後の値`）、主キーが無い・ビュー・主キーが ENUM/SET/FLOAT 等
（`KEYSET_SAFE_TYPES` 以外）のときは1本のSELECTを1つのコネクションでストリーミングする。
ストリーミングを途中で打ち切ったコネクションは、残りの結果が流れてくるためプールへ戻さず破棄する。
...
**GRANTの直後に接続プールを張り直す必要は無い。** プール内のコネクションは `USE` 済みにならず
既定DBも持たないため、完全修飾名でのアクセスは毎回ACLを引き直す（GRANT前から張っていた
コネクションで新DBが見えることを実測済み。#91）。**この前提を保つため、SQL実行画面（`executeSql()`）
が `USE` したコネクションは、使い終えたら `release()` ではなく `destroy()` する**（#139）。
プールへ戻すと、カレントDBへの権限がセッションにキャッシュされたまま残り、GRANT の変更が
届かないコネクションができる。
...
**メタデータDB（`DB_NAME`）は、画面の選択肢・自動登録に加えて `databaseNameSchema`
（`src/lib/config.ts`）でも拒否する**（#140）。「既存DBを登録」「DBを作成」の両アクションと
`createDatabaseEntry` がこのスキーマを通る。画面の選択肢から除くだけでは、細工したPOSTで
サーバーアクションへ直接届く。登録できると、管理ロールの `` `app\_%` `` がこのアプリ自身の
セッション・監査ログのDBにも掛かり、DBユーザー画面から任意の `app_` ユーザーへ権限を付与できてしまう。
`DB_NAME` は検証のたびに読む（`tests/register-metadata-db.test.ts` が `vi.stubEnv` で差し替える）。
